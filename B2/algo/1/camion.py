...
import math
import os
def start():start=input(" la ville de départ : ")
finish=input("la ville d'arrivée : ")
return start,finish
def trajet(start,finish):if(start in villes):if(finish in villes):vitesse,secondes,heures,minutes,distanceparcourue,pauses,pause,lastpause=0,0,0,0,0,0,False,0
lastpause=0
distance=villes[start][finish]
print("Distance entre les villes : ",distance,"km")
while(distanceparcourue<distance*1000):secondes+=1
if(secondes%60==0):if(vitesse<90):if(pause==False):vitesse+=10
print('Accélération')
if((secondes-lastpause)%7200==0):pause=True
pauses+=1
if(pause and secondes%60==0):vitesse-=10
print('Décélération')
if(pause and vitesse==0):secondes+=60*15
lastpause=secondes
pause=False
...
if(secondes/60>0):if(secondes/3600>0):heures=math.floor(secondes/3600)
